    ...
    <div class="login-box" v-loading="loading">
      <div class="logo-box">
        <div class="right">
          ...
          <div class="sys-sub-name">勤勤恳恳做事，踏踏实实做人</div>
        </div>
      </div>
      <div class="form-box">
        <div class="form-item">
          <img src="@/assets/images/shouji.png" >
          <el-input v-model="form.userAccount" type="text" placeholder="请输入账号" />
...
import { loginServeice } from '@/apis/login'
import { setToken } from '@/utils/auth'

// 获取路由对象
let router = useRouter()
let showErrorTip = ref(false)
// 加载圈圈
...
const loginFun = async () => {
  ...
  // 判断是否正确填写表单正确则调用登录接口 
  let { userAccount, password } = form.value
  if (userAccount && password) {
    // 密码登录
    // 接口调用前
    try {
      loading.value = true
      const loginRef = await loginServeice(userAccount, password)
      if (loginRef.data) {
        setToken(loginRef.data)
        router.push('/oj/cuser')
      } else {
        ElMessage({ message: '登录失败，未获取到token', type: "error" });
      }
      loading.value = false
    } catch {
      loading.value = false
