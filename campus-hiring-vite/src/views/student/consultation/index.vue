      ...
      <div class="session-list" v-loading="sessionLoading">
        ...
        >
          <div class="session-avatar">
            <el-avatar :src="session.enterpriseLogo" :size="50">
          ...
          <div class="session-content">
            <div class="session-header">
              <span class="session-name">{{ session.enterpriseName }}</span>
    ...
    <div class="chat-panel" v-if="currentSession">
      ...
      <div class="chat-header">
        <div class="chat-header-info">
          <el-avatar :src="currentSession.enterpriseLogo" :size="40">
            <el-icon><OfficeBuilding /></el-icon>
          </el-avatar>
          <div class="chat-header-text">
            <div class="chat-header-name">
              {{ currentSession.enterpriseName }}
<!--              <span-->
...
<!--              <span v-else class="offline-status">离线</span>-->
      ...
      >
        ...
        >
          <el-avatar
            :src="isSentMessage(message) ? userAvatar : currentSession.enterpriseLogo"
            :size="36"
          >
    ...
    >
      <el-radio-group v-model="selectedResumeId">
        ...
        >
          {{ resume.resumeName }}
          <el-tag v-if="resume.isDefault === 1" size="small" type="success" style="margin-left: 8px">
            默认
          </el-tag>
        </el-radio>
      </el-radio-group>
...
import {
  ...
  type SessionInfo,
  type MessageInfo
} from '@/api/student/consultation'
import { fetchResumeList, type ResumeListItem } from '@/api/student/resume'
import { uploadStudentFile, getDetail } from '@/api/student/student'
import { submitFeedback } from '@/api/student/consultation'
import { submitComplaint } from '@/api/student/complaint'
...
const filteredSessions = computed(() => {
  ...
  return sessions.value.filter(session =>
    session.enterpriseName.toLowerCase().includes(keyword) ||
...
const handleImageSelect = async (file: any) => {
  ...
  try {
    // 上传图片
    const uploadRes = await uploadStudentFile(file.raw)
...
const handleSendResume = async () => {
  if (!currentSessionId.value || !selectedResumeId.value) {
    return
  }
...
const handleSubmitComplaint = async () => {
  ...
  try {
    ...
    if (complaintForm.value.attachment) {
      const uploadRes = await uploadStudentFile(complaintForm.value.attachment)
    ...
    await submitComplaint({
      enterpriseId: currentSession.value.enterpriseId,
...
const isSentMessage = (message: MessageInfo) => {
  ...
  // 确保 userId 和 message.senderId 都存在且类型一致
...
const initWebSocket = () => {
  const userId = userStore.userInfo?.id
  if (!userId) {
    console.warn('[STUDENT][WS] Missing user id, skip websocket init')
    return
  }

  const wsUrl = import.meta.env.DEV
    ? 'ws://localhost:8082/ws'
    : `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.hostname}:8082/ws`

  wsClient = createWebSocketClient({
    url: wsUrl,
    destinations: `/queue/chat/1/${userId}`,
...
onMounted(async () => {
    ...
    try {
      ...
      if (detailRes.code === 1000 && detailRes.data) {
        userStore.setUserInfo({
          ...
          username: detailRes.data.username || userStore.userInfo?.username,
          name: detailRes.data.realName || detailRes.data.nickName || userStore.userInfo?.name,
          avatar: detailRes.data.headImage || userStore.userInfo?.avatar
        })
      }
    } catch (error) {
  ...
  // 如果路由参数中有 enterpriseId 和 jobId，创建或获取会话
  ...
  if (enterpriseId && jobId) {
    try {
      ...
      if (response.code === 1000 && response.data) {
        // 刷新会话列表并选中新创建的会话
        await loadSessions()
        selectSessionById(response.data.id)
      }
    } catch (error) {
...
watch(() => route.query, async (newQuery) => {
  if (newQuery.enterpriseId && newQuery.jobId) {
    try {
      const response = await createOrGetSession({
        enterpriseId: Number(newQuery.enterpriseId),
        jobId: Number(newQuery.jobId)
      })
      if (response.code === 1000 && response.data) {
        await loadSessions()
        selectSessionById(response.data.id)
