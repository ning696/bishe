...
public class EmployeeServiceImpl implements IEmployeeService {
    ...
    @Override
    public ServerResponse updatePassword(Long id, String oldPass, String newPass) {
    ...
    @Override
    public ServerResponse updateCustomerPassword(Long id, String oldPass, String newPass) {
        int resultCount = employeeMapper.updateCustomerPasswordByPrimaryKeyAndOldPass(id, oldPass, newPass);
        if (resultCount > 0) {
            return ServerResponse.createBySuccess();
        }
        return ServerResponse.createByErrorMessage("更新失败");
    ...
    @Override
    public ServerResponse validCustomerPassword(Long id, String validPass) {
        String password = employeeMapper.selectCustomerPasswordByPrimaryKey(id);
        if (password.equals(validPass)) {
            return ServerResponse.createBySuccess();
        }
        return ServerResponse.createByError();
    ...
    private Long createEmployeeId() {
        ...
        String format = dateFormat.format(new Date()) + "700";
        return (long) (Integer.valueOf(format) + (num++));
    }
