...
import com.gxyan.controller.StoreController;
...
import static com.gxyan.util.Carutil.parseCarJson;
...
public class StoreServiceImpl implements IStoreService {
    ...
    @Override
    public ServerResponse addStore(String carJson,List<MultipartFile> files) {
        Car car = Carutil.parseCarJson(carJson);  // 需要实现parseCarJson方法
        ...
        for (MultipartFile file : files) {
            ...
            // 存放上传图片的文件夹
            File fileDir = Carutil.getImgDirFile(car.getSeriesId().toString());
            ...
            try {
                ...
                // 保存图片路径到数据库
                car.setImgPath(car.getSeriesId().toString() + "/" + filename);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    @Override
    public ServerResponse insertPromotion(String promotionText, Long carId) {
        int i = 0;
        try {
            i = carMapper.insertPromotion(promotionText, carId);
        } catch (Exception e) {
            return ServerResponse.createByErrorMessage("已经设置为了宣传");
        }
        if (i != 0) {
            return ServerResponse.createBySuccess();
        }else {
