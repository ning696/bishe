 ...
 */
@Tag(name = "即时聊天（企业端）", description = "企业端即时聊天相关接口")
@RestController
@RequestMapping("/enterprise/chat")
public class ChatController extends BaseController {
     ...
     * 会话列表查询
     */
    @Operation(summary = "会话列表查询", description = "查询企业的会话列表，支持搜索")
    ...
    public R<TableDataInfo> getSessionList(@RequestParam(required = false) String keyword,
        ...
        return chatService.getSessionList(enterpriseId, pageNum, pageSize, keyword);
    }
    ...
    public R<ChatSessionDetailVO> getSessionDetail(@RequestParam Long sessionId,
        ...
        return chatService.getSessionDetail(sessionId, enterpriseId);
    }

    /**
     * 发送消息
     */
    @Operation(summary = "发送消息", description = "发送聊天消息，支持文本、图片、文件、职位信息等类型")
    ...
    public R<ChatMessageSendVO> sendMessage(@RequestBody ChatMessageSendDTO dto,
        ...
        return chatService.sendMessage(dto, enterpriseId);
    }
    ...
    public R<TableDataInfo> getMessageList(@RequestParam Long sessionId,
        ...
        return chatService.getMessageList(sessionId, enterpriseId, pageNum, pageSize);
    }
    ...
    public R<Void> markRead(@RequestBody ChatMessageMarkReadDTO dto,
        ...
        return chatService.markRead(dto, enterpriseId);
    }
    ...
    public R<Void> markReadBySessionId(@PathVariable("sessionId") Long sessionId,
        ...
        return chatService.markReadBySessionId(sessionId, enterpriseId);
    }

    /**
     * 发送职位信息
     */
    @Operation(summary = "发送职位信息", description = "在聊天中发送职位信息")
    @PostMapping("/message/send-job")
    public R<ChatMessageSendVO> sendJob(@RequestBody ChatJobSendDTO dto,
                                        @RequestHeader("Authorization") String authorization) {
        Long enterpriseId = getEnterpriseId(authorization);
        if (enterpriseId == null) {
            return R.fail(ResultCode.FAILED_UNAUTHORIZED);
        }
        return chatService.sendJob(dto, enterpriseId);
    }

    /**
     * 查询未读消息总数
     */
    @Operation(summary = "查询未读消息总数", description = "查询企业的未读消息总数")
    ...
    public R<ChatUnreadCountVO> getUnreadCount(@RequestHeader("Authorization") String authorization) {
        ...
        return chatService.getUnreadCount(enterpriseId);
    }
